...
export default class PciProjectInstanceService {
  ...
  getGrapgQlIntances(projectId, fetchPolicy = 'cache-first') {
    return this.apollo
      .query({
  ...
  getGrapgQlIntance(projectId, instanceId, fetchPolicy = 'cache-first') {
    return this.apollo
      .query({
  ...
  refetchGraphqlInstances(projectId) {
    return this.getGrapgQlIntances(projectId, 'network-only');
  }
}
